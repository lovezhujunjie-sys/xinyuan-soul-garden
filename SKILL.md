...
2. **呼吸静心**（`view-breath`）— 三种节奏：放松 4-7-8 / 平衡 4-4-4-4 / 安眠 4-6。圆圈随吸气放大、呼气缩小，倒计时 + 计轮数。逻辑 `BREATH_MODES` / `runPhase()`。未开始时圆圈有 7s 缓慢「呼吸」idle 脉动（`@keyframes breatheIdle`，仅 `prefers-reduced-motion:no-preference`）邀请人停下来；结束时按完成轮数给一句温柔收束语（`breathBless(n)`，1-2/3-5/6+ 三档），让平静留在心里。<br>**5-4-3-2-1 着陆练习**（`.ground-card` + `groundStep()` / `GROUND_STEPS`）：呼吸卡下方一张卡，焦虑慌乱时用五感（看见5/听见4/触感3/气味2/味道1）把注意力一步步带回此时此地，点按推进、结尾「你已回到此时此地，这里是安全的」并可「再做一次」。这是循证的急性焦虑安抚法，纯静态文案（`<b>` 强调、无用户输入），直击「平安喜乐 + 全人类温柔可及」。<br>**冥想音景**（`window.Ambient` 模块 + `#ambPills`）：雨/海浪/风/颂钵四种环境音，**全部用 Web Audio 实时合成**（噪声 buffer + BiquadFilter 塑形 / 多泛音 oscillator + LFO 调制），零音频文件、离线可用；同一时刻放一个、平滑交叉淡入淡出，配音量滑块。AudioContext 懒初始化（首次点击 pill 的用户手势内创建并 resume），切走该 tab 时自动 `Ambient.stop()`。
