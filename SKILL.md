...
疗愈系深紫夜空背景 + 漂浮光晕（`.orb`）+ 毛玻璃卡片（`backdrop-filter`）。**夜间柔光**：`applyNightMode()`（init 调用）在 22 点后/6 点前给 `body` 加 `.night` 类，调暗光晕 + 柔和压暗边缘 + 略降亮度，深夜护眼、陪人慢慢入睡，与「夜深了」问候呼应。配色变量在 `:root`：`--accent` 紫 / `--accent2` 青 / `--accent3` 暖橙。底部六 tab 导航（每日/呼吸/心情/养护/成长/社区）。视觉与文案都应延续「平安、喜乐、自在、温柔」的基调。**气质收尾细节**：标题「心园」用青→白→紫渐变文字（`background-clip:text`）+ 头顶柔光晕（`header.top::before`）；首页每日卡有 9s 缓慢「呼吸」光晕（`@keyframes dailyBreathe`，仅 `prefers-reduced-motion:no-preference`）；底部导航选中态有上浮放大图标 + 发光小圆点（`.nav button.on::after`）；按钮带 `:active` 轻压回馈；全局柔和滚动条 + 紫色 `::selection` + 字体抗锯齿；卡间距 18px 留白。新增视觉请延续这套"静、柔、有呼吸感"的细节。
